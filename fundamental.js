...
function carregarProjetoFromContent(contentStr, isJson) {
    if (isJson) {
        try {
            ...
            carregarTurmasMontador();
            renderizarTabelaMontador();
            alert("Projeto Fundamental carregado com sucesso!");
        } catch (err) {
            console.error(err);
...
function handleLocalImport(file) {
    if (!file) return;
    let modal = document.getElementById("drive-modal-fund");
...
function abrirModalArquivos() {
    ...
    modal.innerHTML = `
        <div style="background:#fff; border-radius:12px; padding:20px; width:450px; max-width:90%; position:relative;">
            ...
            <h3 style="margin-top:0; color:#333; font-family:Inter, sans-serif;">Abrir Projeto Fundamental</h3>
            ...
            <p style="color:#555; font-size:14px; text-align:center;">Ou abra de um arquivo local (.json ou .csv)</p>
    ...
    fetch(GOOGLE_APPS_SCRIPT_URL + "?action=list")
      ...
      .then(files => {
          ...
          if (!files || files.length === 0) {
              html = "<em style='color:#888;'>Nenhum projeto encontrado.</em>";
          } else {
              files.forEach(f => {
      ...
      .catch(err => {
          document.getElementById('drive-file-list').innerHTML = "<span style='color:red;'>Erro ao carregar arquivos da nuvem. Verifique a configuração do seu script.</span>";
      });
}
...
function salvarProjetoDrive() {
    ...
    if (nomeBase.trim() === "") nomeBase = "Projeto_Montador_Fundamental";
    ...
    if (GOOGLE_APPS_SCRIPT_URL) {
        ...
        alert("Como não há script de Backend Drive configurado, iremos baixar o arquivo localmente.");
        var blob = new Blob([jsonStr], {type: "application/json;charset=utf-8"});
        saveAs(blob, nomeFinal);
    }
}
